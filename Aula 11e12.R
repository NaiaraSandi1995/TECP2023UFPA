...
Model1.1 <- lm(votos ~ TOTAL_RECEITAS, data = sen2018)
...
tab_model(Model1.1, show.ci = F, auto.label = T, 
          collapse.se = T, wrap.labels = 100, p.style = "stars" )
...
resid <- (cbind(sen2018$votos, predict(Model1.1),
                residuals(Model1.1)))
...
sen2018[285, c(3, 15, 16)] #trabalhar com o modelo sem logaritmizar
# SERGIO OLIMPIO GOMES 9.039.717        380.324.
...
sen2018 <- sen2018 %>%
  mutate(Instrução = case_when(
    Instrução %in% c("Baixa", 
                     "Média") ~ "Média e Baixa",
    Instrução %in% c("Superior") ~ "Superior",
    TRUE ~ Instrução
  ))
table(sen2018$Instrução)
...
pairs.panels(GraficoModelos,
             method = "pearson",
             hist.col = "#00AFBB")
#A diagonal mostra um histograma de cada variável.

#*mostrar as correlações indicando-se der tempo
...
Gra1 <- ggplot(data = sen2018, 
               aes(y = log.votos, x= log.receitas,
                   size = log.votos, 
                   color =log.receitas)) +
  geom_point(alpha = 0.5) +
...
EquaRet <- lm(log.votos ~ log.receitas, data = sen2018)
summary(EquaRet)
